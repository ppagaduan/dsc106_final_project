...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t1 = space();
    			input = element("input");
    			add_location(label, file, 191, 8, 5453);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1");
    			attr_dev(input, "max", "1440");
    			add_location(input, file, 192, 2, 5485);
    			attr_dev(div, "class", "overlay");
    			add_location(div, file, 189, 1, 5420);
    			add_location(main, file, 188, 0, 5412);
    		},
    ...
    function scaleColorTrafficVolume(traffic, maxTraffic = 500) {
    	const scaleColor = d3.scaleLinear().domain([1, maxTraffic]).range(["brown", "steelblue"]);
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	function create_station_markers(station_data) {
    		station_markers = marker_container.selectAll("circle").data(station_data).enter().append("circle").attr("r", 5).attr("stroke", "#808080").attr("stroke-width", 1).attr("fill-opacity", 0.4).attr("name", function (d) {
    			return d["name"];
    		});
    	...
    	function update_station_markers() {
    		station_markers.transition().duration(1000).attr("r", function (d) {
    			...
    			let trafficVolume = arrivals[d["station_id"]] + departures[d["station_id"]];
    			return scaleColorTrafficVolume(trafficVolume);
    		});
    	}
